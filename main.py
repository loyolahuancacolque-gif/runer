...
def fondo_nivel(lv):
    if lv not in CACHE_FONDO:
        top, bot = TEMAS[lv - 1]
        s = gradiente(ANCHO, 350, top, bot)
        suelo = gradiente(ANCHO, 50, SUELOS[lv - 1], tuple(max(0, c - 38) for c in SUELOS[lv - 1]))
        s = s.copy(); base = pygame.Surface((ANCHO, ALTO)); base.blit(s, (0, 0)); base.blit(suelo, (0, 350))
        CACHE_FONDO[lv] = base
    return CACHE_FONDO[lv]

def draw_bg(lv):
    VENTANA.blit(fondo_nivel(lv), (0, 0))
    p1 = int(scroll * .18) % 900; p2 = int(scroll * .42) % 480
    if lv == 1:
        brillo(660, 70, 60, (255, 240, 180), 90)
        pygame.draw.circle(VENTANA, (255, 248, 210), (660, 70), 26)
        for i in range(4):
            cx = i * 300 - p1
            pygame.draw.ellipse(VENTANA, (150, 200, 140), (cx, 250, 380, 130))
            pygame.draw.ellipse(VENTANA, (118, 178, 112), (cx + 120, 268, 340, 120))
        for xx in range(-60, 860, 120):
            tx = xx - p2
            pygame.draw.rect(VENTANA, (78, 54, 32), (tx + 50, 190, 16, 160), border_radius=4)
            pygame.draw.circle(VENTANA, (26, 96, 48), (tx + 58, 178), 40)
            pygame.draw.circle(VENTANA, (44, 132, 62), (tx + 46, 168), 28)
    elif lv == 2:
        pygame.draw.polygon(VENTANA, (24, 24, 32), [(0, 0), (800, 0), (800, 84), (700, 56), (620, 92), (500, 60), (380, 92), (250, 56), (120, 92), (0, 60)])
        for xx in range(30, 800, 80):
            pygame.draw.polygon(VENTANA, (72, 72, 82), [(xx, 55), (xx + 25, 55), (xx + 12, 96 + (xx % 40))])
            pygame.draw.polygon(VENTANA, (54, 54, 64), [(xx - p2 % 80, 350), (xx + 20 - p2 % 80, 300), (xx + 44 - p2 % 80, 350)])
        brillo(140, 120, 50, (255, 180, 90), 45)
    elif lv == 3:
        for xx in range(0, 900, 150):
            bx = xx - p2
            pygame.draw.rect(VENTANA, (48, 54, 64), (bx, 110, 110, 240))
            pygame.draw.rect(VENTANA, (66, 72, 82), (bx, 110, 110, 10))
            pygame.draw.rect(VENTANA, (74, 74, 80), (bx + 75, 45, 25, 100))
            for yy in range(140, 340, 40):
                pygame.draw.rect(VENTANA, (255, 200, 90), (bx + 16, yy, 12, 14), border_radius=2)
    elif lv == 4:
        for i in range(11):
            bx = i * 90 - p2; h = 100 + (i * 37) % 150
            pygame.draw.rect(VENTANA, (26, 30, 66), (bx, 350 - h, 70, h))
            pygame.draw.rect(VENTANA, (44, 52, 104), (bx, 350 - h, 70, 4))
            for yy in range(350 - h + 20, 330, 30):
                pygame.draw.rect(VENTANA, (70, 220, 255), (bx + 15, yy, 8, 12))
        brillo(120, 90, 70, (90, 120, 255), 50)
    elif lv == 5:
        for i in range(80):
            sx = (i * 137 - scroll * .35) % 800; sy = (i * 71) % 300
            pygame.draw.circle(VENTANA, (215, 220, 255), (int(sx), sy), 1 if i % 3 else 2)
        brillo(680, 100, 70, (90, 110, 220), 70)
        pygame.draw.circle(VENTANA, (78, 90, 190), (680, 100), 45)
        pygame.draw.circle(VENTANA, (58, 70, 160), (666, 88), 14)
    elif lv == 6:
        brillo(680, 70, 60, (220, 70, 60), 80)
        pygame.draw.circle(VENTANA, (205, 50, 45), (680, 70), 34)
        pygame.draw.rect(VENTANA, (40, 30, 42), (170, 130, 460, 220))
        for xx in [120, 250, 520, 650]:
            pygame.draw.rect(VENTANA, (52, 38, 50), (xx, 100, 70, 250))
            pygame.draw.polygon(VENTANA, (32, 18, 34), [(xx - 12, 100), (xx + 35, 52), (xx + 82, 100)])
            for yy in range(140, 330, 50):
                pygame.draw.rect(VENTANA, (255, 190, 70), (xx + 28, yy, 12, 18), border_radius=3)
    elif lv == 7:
        for i in range(10):
            mx = i * 100 - p2
            pygame.draw.polygon(VENTANA, (84, 24, 10), [(mx, 350), (mx + 45, 190 + (i * 23) % 50), (mx + 90, 350)])
            pygame.draw.polygon(VENTANA, (120, 38, 14), [(mx + 45, 190 + (i * 23) % 50), (mx + 66, 246), (mx + 30, 250)])
        for i in range(14):
            fx = (i * 97 + frame * 2) % 800; fy = 320 - (frame * 2 + i * 60) % 260
            pygame.draw.circle(VENTANA, (255, 130 + (i * 13) % 90, 40), (int(fx), int(fy)), 2 + i % 3)
    else:
        for i in range(9):
            pygame.draw.ellipse(VENTANA, (44 + i * 8, 15, 86 + i * 10), (i * 100 - 60 - p2 // 2, 60 + (i % 3) * 50, 180, 100), 3)
        for i in range(40):
            sx = (i * 191 - scroll * .5) % 800
            pygame.draw.circle(VENTANA, (200, 150, 255), (int(sx), (i * 53) % 300), 1)
    # Línea de suelo con textura en movimiento
...
def draw_dino(px, py, col, kind='clasico', fr=0, enemy=False):
    c = col; osc = tuple(max(0, v - 55) for v in c); cl = tuple(min(255, v + 45) for v in c)
    if not enemy: sombra_suelo(px + 4, 344, 46, 8, 70 if py < 290 else 95)
    if (not enemy) and agachado:
        pygame.draw.rect(VENTANA, c, (px + 8, py + 22, 40, 18), border_radius=7)
        pygame.draw.rect(VENTANA, cl, (px + 10, py + 24, 34, 5), border_radius=3)
        pygame.draw.rect(VENTANA, c, (px + 28, py + 12, 22, 14), border_radius=6)
        pygame.draw.polygon(VENTANA, osc, [(px + 2, py + 27), (px + 16, py + 24), (px + 16, py + 36)])
        pygame.draw.rect(VENTANA, osc, (px + 18, py + 37, 7, 6), border_radius=2)
        pygame.draw.rect(VENTANA, osc, (px + 34, py + 37, 7, 6), border_radius=2)
        pygame.draw.circle(VENTANA, (255, 255, 255), (px + 45, py + 17), 4)
        pygame.draw.circle(VENTANA, (18, 18, 22), (px + 46, py + 17), 2)
        return
    cola = math.sin(fr * .25) * 4
    pygame.draw.polygon(VENTANA, osc, [(px + 12, py + 22), (px - 14, py + 20 + cola), (px + 12, py + 34)])
    pygame.draw.rect(VENTANA, c, (px + 10, py + 14, 28, 27), border_radius=9)
    pygame.draw.rect(VENTANA, cl, (px + 14, py + 17, 18, 6), border_radius=3)
    pygame.draw.rect(VENTANA, osc, (px + 12, py + 32, 24, 8), border_radius=4)
    pygame.draw.rect(VENTANA, c, (px + 22, py, 26, 21), border_radius=8)
    pygame.draw.rect(VENTANA, osc, (px + 35, py + 14, 15, 7), border_radius=3)
    pygame.draw.rect(VENTANA, cl, (px + 26, py + 3, 14, 5), border_radius=3)
    pygame.draw.rect(VENTANA, osc, (px + 30, py + 24, 12, 6), border_radius=3)
    if py < 300: legs = [(px + 15, py + 39, 7, 8), (px + 29, py + 40, 7, 7)]
    elif fr % 12 < 6: legs = [(px + 12, py + 38, 7, 12), (px + 28, py + 39, 7, 7)]
    else: legs = [(px + 12, py + 40, 7, 7), (px + 28, py + 37, 7, 12)]
    for r in legs: pygame.draw.rect(VENTANA, osc, r, border_radius=3)
    pygame.draw.circle(VENTANA, (255, 255, 255), (px + 41, py + 7), 5)
    pygame.draw.circle(VENTANA, (18, 18, 22), (px + 42, py + 7), 2)
    if kind == 'triceratops':
        pygame.draw.polygon(VENTANA, cl, [(px + 42, py + 4), (px + 52, py - 6), (px + 48, py + 8)])
        pygame.draw.polygon(VENTANA, cl, [(px + 34, py + 4), (px + 29, py - 6), (px + 32, py + 8)])
    if kind == 'rex':
        pygame.draw.polygon(VENTANA, (255, 250, 235), [(px + 44, py + 16), (px + 58, py + 21), (px + 44, py + 23)])
        for dx in range(12, 34, 8):
            pygame.draw.polygon(VENTANA, cl, [(px + dx, py + 15), (px + dx + 5, py + 6), (px + dx + 9, py + 16)])
    ...
    for p in proyectiles[:]:
        ...
        if pygame.Rect(int(p['x'] - 8), int(p['y'] - 8), 16, 16).colliderect(pygame.Rect(int(jefe['x']), int(jefe['y']), 150, 150)):
            ...
            if jefe['hp'] <= 0:
                ...
                tipo_victoria = 'pelea'; save_score(puntos, tipo_victoria)
